...
void setup_server(int num_cl, u_short port) {
  ...
  */
  ...
  if (bind(rsock, (struct sockaddr *)&sv_addr, sizeof(sv_addr)) != 0) { //ソケットに設定を結びつける
    handle_error("bind()"); //エラーを表示
  }
  //実行に成功したことを表示
  fprintf(stderr, "bind() is done successfully.\n");

  if (listen(rsock, num_clients) != 0) { //接続ソケットの準備，引数1は，接続要求受付用ソケット
...
static void send_data(int cid, void *data, int size) {
  ...
  if (cid == BROADCAST) { //送信先がすべてのクライアントのとき
    int i;
    for (i = 0; i < num_clients; i++) { //すべてのクライアントに対して，クライアントデータを送信
    ...
    */ 
    if (write(clients[cid].sock, data, size) < 0) { //送信先が指定されているときは，指定のクライアントにクライアントデータを送信
      handle_error("write()");
    }
